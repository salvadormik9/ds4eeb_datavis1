## Learning Outcomes 
By the end of this activity, students will be able to:
1. Understand the difference between hex codes and RGB
2. Import and apply different colors and color palettes in R
3. Recognize how color choices affect data interpretation
4. Carefully choose colors to make plots more accessible

## Why should we care about color in science?

1. Aesthetics
  * Make your research more compelling and engaging
  * Bring out patterns in your data more clearly
2. Storytelling
  * Can create a throughline in your papers and presentations
  * Can help reinform concepts for audience comprehension (e.g., hot treatment in RED, cold treatment in BLUE)
3. Accessibility
  * Can broaden access to your research
  * Around 1/12 men and 1/200 women experience color vision deficiencies!

## Activity: Using Color in R
In this activity, you will practice loading colors into R using built-in color names, HEX codes, and R package color palettes.

To download the .R file, click on `datavis1_activity.R`. This will bring you to a new page. 
Near the top right, click the Download button. It looks like a downward arrow.

<img width="584" height="166" alt="DownloadingRfiles" src="https://github.com/user-attachments/assets/01f55475-f532-400b-96f6-06ad8c38140c" />

Once downloaded to your computer, you can now open `datavis1_activity.R` in RStudio! This activity should take ~ 15 minutes.

## Resources
* Our presentation [slides](https://docs.google.com/presentation/d/1yfYj6pq6YVQREUcFA_4Xk0ZHgGvORrsM_tWqQLC6bmU/edit?usp=sharing)
* Resource for deciding best color palettes for data visualization from [Simplified Science Publishing](https://www.simplifiedsciencepublishing.com/resources/best-color-palettes-for-scientific-figures-and-data-visualizations)
* A color palette [cheat-sheet for R](https://www.nceas.ucsb.edu/sites/default/files/2020-04/colorPaletteCheatsheet.pdf)
* A great website for finding ready-to-use color palettes: [color palette finder](https://r-graph-gallery.com/color-palette-finder)
* A great website for generating **new** color palettes [coolors](https://coolors.co/)

After this activity, you will have better tools to pick appropriate colors for your figures to better communicate your science!

Example of poor color choice:
<img width="385" height="275" alt="Rplot_badcolors" src="https://github.com/user-attachments/assets/b4f691ca-b689-4ff7-bed4-0ec40adda165" />

Example of better color choice:
<img width="385" height="275" alt="Rplot_goodcolors" src="https://github.com/user-attachments/assets/d44849c9-0e51-4d2e-83ba-fcdeb9b48cd9" />


